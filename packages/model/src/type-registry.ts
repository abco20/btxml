...
export function isBlackboardPointer(
  value: string,
  strippedPointer?: { current?: string },
): boolean {
  if (value.length < 3) {
    return false;
  }
...
export function stripBlackboardPointer(value: string): string | undefined {
  const strippedPointer: { current?: string } = {};
  return isBlackboardPointer(value, strippedPointer) ? strippedPointer.current : undefined;
}

export function getRemappedKey(portName: string, rawValue: string): string | undefined {
