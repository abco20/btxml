...
test("reports BT120 when same model ID uses different kinds", async () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ files: { include: ["*.xml"] }, models: { convention: "allow-unused" } }),
    "utf8",
  );
  fs.writeFileSync(
...
test("used-only reports unused inline normal models and ignores SubTree", async () => {
  ...
  fs.writeFileSync(
    path.join(dir, "btxml.config.json"),
    JSON.stringify({ files: { include: ["*.xml"] }, models: { convention: "used-only" } }),
    "utf8",
  );
  fs.writeFileSync(
    path.join(dir, "tree.xml"),
...
test("used-only requires usage in the same file", async () => {
  ...
  assert.equal(
    allDiagnostics(result).some((diagnostic) => diagnostic.code === "BT121_UNUSED_MODEL_DEFINITION"),
    true,
  );
});
...
test("single-source reports duplicate user definitions", async () => {
  ...
  fs.writeFileSync(
    path.join(dir, "btxml.config.json"),
    JSON.stringify({ files: { include: ["*.xml"] }, models: { convention: "single-source" } }),
    "utf8",
  );
  fs.writeFileSync(
    path.join(dir, "a.xml"),
  ...
  assert.equal(
    allDiagnostics(result).some((diagnostic) => diagnostic.code === "BT122_DUPLICATE_MODEL_DEFINITION"),
    true,
  );
});
