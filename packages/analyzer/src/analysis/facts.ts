...
export function emptyAnalysisFacts(): AnalysisFacts {
  return { includeIssuesByUri: new Map(), suppressionIssuesByUri: new Map() };
}

export function getIncludeIssuesForUri(facts: AnalysisFacts, uri: string): IncludeIssue[] {
  return facts.includeIssuesByUri.get(uri) ?? [];
}

export function getSuppressionIssuesForUri(facts: AnalysisFacts, uri: string): SuppressionIssue[] {
  return facts.suppressionIssuesByUri.get(uri) ?? [];
}
